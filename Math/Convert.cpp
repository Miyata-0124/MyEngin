...
Matrix4 Convert::ConvertXMMATtoMat4(XMMATRIX XMMat) {
	Matrix4 result;
	for (int i = 0; i < 4; i++) {
...
XMMATRIX Convert::ConvertMat4toXMMAT(Matrix4 m) {
	XMMATRIX result;
	result = XMMatrixSet(
...
Vector3 Convert::ConvertXMFlo3toVec3(XMFLOAT3 XMFlo3) {
	Vector3 result;
	result.x = XMFlo3.x;
	result.y = XMFlo3.y;
	result.z = XMFlo3.z;
...
XMFLOAT3 Convert::ConvertVec3toXMFlo3(Vector3 vec3) {
	XMFLOAT3 result;
	result.x = vec3.x;
	result.y = vec3.y;
	result.z = vec3.z;
...
Vector4 Convert::XMVECToVec4(XMVECTOR XMVEC) {
	Vector4 result;
...
XMVECTOR Convert::Vec4ToXMVEC(Vector4 Vec4) {
	XMVECTOR result;

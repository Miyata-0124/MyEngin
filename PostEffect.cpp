...
void PostEffect::CreateTex() {
	...
	for (size_t i = 0; i < 2; i++)
	{
		...
		{//テクスチャを赤クリア
			...
			//画像1行分のデータサイズ
			const uint32_t depthPitch = sizeof(uint32_t) * WinApp::window_height;
			//画像イメージ
...
void PostEffect::CreateDepth() {
	...
	CD3DX12_RESOURCE_DESC depthResourceDesc =
		CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_D32_FLOAT, WinApp::window_width, WinApp::window_height, 1, 0, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
	//深度バッファの生成
...
void PostEffect::CreateGraphicsPipelineState() {
	...
	result_ = D3DCompileFromFile(
		L"Resources/hlsl/PostEffectVS.hlsl",
		nullptr,
	...
	result_ = D3DCompileFromFile(
		L"Resources/hlsl/PostEffectPS.hlsl",
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main", "ps_5_0",
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION,
...
void PostEffect::PreDrawScene(ID3D12GraphicsCommandList * cmdList_) {
	...
	for (size_t i = 0; i < 2; i++)
	{
		viewPort[i] = CD3DX12_VIEWPORT(0.0f, 0.0f, static_cast<float>(WinApp::window_width), static_cast<float>(WinApp::window_height));
		scissorRects[i] = CD3DX12_RECT(0, 0, static_cast<float>(WinApp::window_width), static_cast<float>(WinApp::window_height));
	}

...
XMMATRIX	Particle::matBillboard = XMMatrixIdentity();
XMMATRIX	Particle::matBillboardY = XMMatrixIdentity();
std::string Particle::defaultTextureDirectoryPath = "Resources/";
...
ID3D12DescriptorHeap* Particle::descHeap;
ComPtr<ID3D12Resource> Particle::vertBuff;
ComPtr<ID3D12Resource> Particle::texBuffers[Particle::maxSRVCount];
...
void Particle::LoadTexture(uint32_t index, std::string fileName)
{
	HRESULT result = S_FALSE;
...
void	Particle::SetTextureCommands(uint32_t index) {
...
void Particle::InitializeGraphicsPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Resources/hlsl/ParticleVS.hlsl",	// シェーダファイル名
		nullptr,
	...
	result = D3DCompileFromFile(
		L"Resources/hlsl/ParticleGS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources/hlsl/ParticlePS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
bool Particle::Initialize(uint32_t texIndex_)
{
	// nullptrチェック
	assert(device);
...
void Particle::Update()
{
		...
		itrator_ != particles.end(); itrator_++)
	{
		...
		itrator_->frame++;
		//速度に加速度を加算
		itrator_->velocity = itrator_->velocity + itrator_->accel;
		//速度による移動
		itrator_->position = itrator_->position + itrator_->velocity;
		//スケールの線形補間
		float	f = (float)itrator_->frame / itrator_->num_frame;
		itrator_->scale = (itrator_->e_scale - itrator_->s_scale) * f;
		itrator_->scale += itrator_->s_scale;
	}
